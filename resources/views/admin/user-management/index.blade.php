<x-app-layout>
             ...
             <div class="bg-gradient-to-r from-red-50 to-orange-50 border-l-4 border-red-500 p-6 rounded-lg shadow-sm mb-6">
                 <div class="flex justify-between items-center">
                     <div>
                        ...
                        <a href="{{ route('profile-user-management.create') }}" 
                     ...
                     <div id="searchFilterContent" class="bg-white border border-gray-200 rounded-lg shadow-sm mb-6" style="display: none;">
                         <div class="p-6">
                             <form id="searchFilterForm" method="GET" action="{{ route('profile-user-management.index') }}" class="space-y-6">
                                ...
                                <thead class="bg-red-600">
                                    <tr>
                                        <th class="px-6 py-3 text-left text-xs font-bold text-white uppercase tracking-wider">
                                            <a href="{{ route('profile-user-management.index', array_merge(request()->query(), ['sort' => 'created_at', 'direction' => request('direction') === 'asc' && request('sort')==='created_at' ? 'desc' : 'asc'])) }}" class="inline-flex items-center gap-1 text-white hover:text-gray-200">
                                                User
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const form = document.querySelector('form[action="{{ route('profile-user-management.index') }}"]');
        ...
        function confirmDeleteUser(userId) {
            ...
            // Make AJAX request
            fetch(`{{ route('profile-user-management.destroy', ':userId') }}`.replace(':userId', userId), {
                method: 'DELETE',
                headers: {
        ...
        function performSearch() {
            ...
            // Make AJAX request
            fetch(`{{ route('profile-user-management.index') }}?${params.toString()}`, {
                method: 'GET',
                headers: {
                    'X-Requested-With': 'XMLHttpRequest',
                    'Accept': 'text/html'
