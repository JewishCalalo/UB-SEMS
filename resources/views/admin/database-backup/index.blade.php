<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-200 rounded-lg shadow-sm mb-6 max-w-4xl mx-auto">
                <div class="p-6">
                    ...
                    <form action="{{ route('admin.database-backup.create') }}" method="POST" class="space-y-4">
                        @csrf
                        ...
                        <div class="flex justify-center pt-2">
                            <x-form-button type="submit" variant="primary" size="md" icon="fas fa-plus">
                                Create Backup
                            </x-form-button>
                        </div>
                    </form>
                                    ...
                                    @foreach($backups as $backup)
                                        <tr class="hover:bg-gray-50">
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="text-sm font-bold text-gray-900">{{ $backup['filename'] }}</div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="text-sm font-medium text-gray-700 max-w-xs truncate" title="{{ $backup['description'] }}">
                                                    {{ $backup['description'] }}
                                                </div>
                                            </td>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                <div class="flex items-center space-x-2">
                                                    <button onclick="viewBackupDetails('{{ $backup['filename'] }}', '{{ $backup['description'] }}', '{{ $backup['size'] }}', '{{ $backup['created'] }}', '{{ $backup['is_encrypted'] ? 'Yes' : 'No' }}', '{{ $backup['created_by'] ?? 'Unknown' }}')" 
                ...
                <div class="items-center px-4 py-3">
                    <form id="restoreForm" method="POST" class="inline">
                        ...
                        <div class="mt-4 space-x-3">
                            <x-form-button type="submit" variant="danger" size="sm">
                                Yes, Restore Database
                            </x-form-button>
                            <x-form-button type="button" variant="secondary" size="sm" onclick="closeRestoreModal()">
                                Cancel
                            </x-form-button>
                        </div>
                    </form>
    ...
    <script>
        ...
        function confirmRestore(filename) {
            document.getElementById('restoreForm').action = '{{ route("admin.database-backup.restore", ":filename") }}'.replace(':filename', filename);
        ...
        function confirmDeleteBackup() {
            ...
            fetch('{{ route("admin.database-backup.delete", ":filename") }}'.replace(':filename', filename), {
                method: 'POST',
                body: formData,
                headers: {
                ...
                Swal.fire({
                    ...
                    html: `
                        ...
                        <div class="text-center">
                            ...
                            <p class="text-gray-700">{{ session('delete_success') }}</p>
                        </div>
                        <div class="flex justify-center mt-6">
                            <button type="button" class="px-6 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 transition-colors transform hover:scale-105" style="border: none !important; outline: none !important;" onclick="Swal.close();">
                                OK
                    ...
                    Swal.fire({
                        ...
                        html: `
                            ...
                            <div class="text-center">
                                ...
                                <p class="text-gray-700 mb-4">{{ $errors->first('password_error') }}</p>
                            </div>
                            <div class="flex justify-center mt-6">
                ...
                @else
                    ...
                    Swal.fire({
                        ...
                        html: `
                            ...
                            <div class="text-center">
                                <p class="text-gray-700 mb-4">{{ $errors->first() }}</p>
                                @if(str_contains($errors->first(), 'custom filename'))
                                    <div class="bg-red-50 border border-red-200 rounded-lg p-4">
