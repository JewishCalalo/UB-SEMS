...
<body>
    ...
    <div class="filters">
        ...
        @if(isset($filters['user']) && $filters['user'])
            <p><strong>User:</strong> {{ $filters['user'] }}</p>
        @endif
        ...
        <tbody>
            @foreach($reservations as $reservation)
                ...
                <tr>
                    ...
                    <td>{{ $reservation->user ? $reservation->user->name : ($reservation->name ?? 'Guest User') }}</td>
                    <td>{{ $reservation->user ? $reservation->user->email : ($reservation->email ?? 'No email') }}</td>
                    <td>{{ $item->equipment->name }} ({{ $item->quantity_requested }}x)</td>
                    <td class="status-{{ $reservation->status }}">{{ ucfirst($reservation->status) }}</td>
                    <td>{{ $reservation->borrow_date->format('M d, Y') }}</td>
                    <td>{{ $reservation->return_date->format('M d, Y') }}</td>
                    <td>{{ Str::limit($reservation->reason, 30) }}</td>
    ...
    <div class="footer">
        <p>Total Records: {{ $reservations->count() }}</p>
