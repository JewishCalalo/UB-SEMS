    ...
    <div class="py-12 bg-rose-50/40">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form method="POST" action="{{ route('instructor.incidents.store') }}" class="space-y-6" id="incidentForm">
                        @csrf
                                ...
                                <select name="reservation_id" id="reservation_id" required
                                    ...
                                    @foreach($reservations as $reservation)
                                        ...
                                        <option value="{{ $reservation->id }}" 
                                                data-equipment="{{ json_encode($approvedEquipment) }}">
                                            {{ $reservation->reservation_code }} - {{ $reservation->borrow_date->format('M d, Y') }} to {{ $reservation->return_date->format('M d, Y') }}
                                        </option>
                                    @endforeach
                                </select>
            ...
            form.addEventListener('submit', function(e) {
                ...
                .then(response => {
                    ...
                    if (contentType && contentType.includes('application/json')) {
                        ...
                        window.location.href = '{{ route('instructor.incidents.index') }}';
                        return;
                    }
                })
                .then(data => {
                        ...
                        Swal.fire({
                            ...
                            html: `
                                ...
                                <div class="flex justify-center mt-6">
                                    <button type="button" class="px-6 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 transition-colors transform hover:scale-105" onclick="Swal.close(); window.location.href='{{ route('instructor.incidents.index') }}'">
                                        OK
                                    </button>
                                </div>
                            `,
