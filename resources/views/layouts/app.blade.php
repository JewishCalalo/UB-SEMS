...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>{{ config('app.name', 'SEMS') }}</title>
        ...
        <link rel="shortcut icon" href="{{ asset('images/ub-logo.png') }}">
        <link rel="apple-touch-icon" href="{{ asset('images/ub-logo.png') }}">
    ...
    <body class="font-sans antialiased" style="background-image: url('{{ asset('images/Background.jpg') }}'); background-size: cover; background-position: center; background-repeat: no-repeat; background-attachment: fixed;">
        <div class="min-h-screen bg-white/500">
            ...
            @if($sectionTitle)
                <x-section-banner :title="$sectionTitle" />
            @endif
            ...
            <div id="notification-container" class="fixed top-4 right-4 z-50 space-y-2">
                @if(session('success'))
                    <script>
                        document.addEventListener('DOMContentLoaded', function() {
                            ...
                            const variant = @json(session('success_variant'));
                            if (window.Swal && variant !== 'banner') {
                                Swal.fire({
                                    icon: 'success',
                                    title: @json(session('success_title') ?? 'Action Completed'),
                                    ...
                                    Swal.fire({
                                        ...
                                        html: `
                                            <div class="bg-gradient-to-r from-green-600 to-emerald-600 text-white p-4 -m-6 -mt-6 mb-4 rounded-t-lg" style="margin-left:-24px;margin-right:-24px;margin-top:-24px;">
                                                <h2 class="text-xl font-bold text-center">` + @json(session('success_title') ?? 'Success') + `</h2>
                                            </div>
                                            <div class="text-center">
                                                ...
                                                <p class="text-gray-700">` + @json(session('success')) + `</p>
                                            </div>
                                        `,
                ...
                {{-- @if(session('error'))
                    <div class="notification error bg-red-500 text-white px-6 py-4 rounded-lg shadow-lg transform transition-all duration-300 translate-x-full">
                ...
                {{-- @if($errors->any())
                    @php
                        ...
                        foreach($errors->getMessages() as $key => $errs){
                            ...
                            } elseif(\Illuminate\Support\Str::contains($key, 'equipment_instances')){
                                $messages['instances'] = 'Please select an instance for each item.';
                            } elseif(\Illuminate\Support\Str::contains($key, 'pickup_conditions')){
                                $messages['pickup_conditions'] = 'Please choose a pickup condition for each selected instance.';
                            } elseif(\Illuminate\Support\Str::contains($key, 'damage_description')){
                                $messages['damage_description'] = 'Add a short damage description for items marked damaged or needs repair.';
                            } elseif(\Illuminate\Support\Str::contains($key, 'condition_notes')){
                                $messages['condition_notes'] = 'Please add brief notes where context is needed.';
                            } else {
                                $messages[] = $errs[0];
                            }
                        }
                    ...
                    <div class="notification error bg-red-500 text-white px-6 py-4 rounded-lg shadow-lg transform transition-all duration-300 translate-x-full">
                        <div class="flex items-start">
                            ...
                            <div>
                                <div class="font-medium">Please review the form:</div>
        ...
        @auth
            ...
            @if(session('status') === 'please-complete-profile')
                <script>
                    document.addEventListener('DOMContentLoaded', function() {
                        Swal.fire({
                            icon: false,
