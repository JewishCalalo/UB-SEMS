...
<head>
    ...
    <title>{{ config('app.name', 'SEMS') }} - Track Reservation</title>
        ...
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8 w-full relative z-10">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-xl border border-gray-200">
                ...
                <div class="p-6 sm:p-8 text-gray-900">
                    ...
                    <!-- Search Form -->
                    <form method="GET" action="{{ route('reservations.track') }}" class="mb-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                <div>
                                    ...
                                    <div class="space-y-2 text-sm text-gray-600">
                                        <p><strong>Name:</strong> {{ $reservation->user ? $reservation->user->name : ($reservation->name ?? 'Guest User') }}</p>
                                        <p><strong>Email:</strong> {{ $reservation->user ? $reservation->user->email : ($reservation->email ?? 'No email') }}</p>
                                ...
                                <div class="space-y-2" id="trackReservationItems">
                                    @foreach($reservation->items as $item)
                                        <div class="flex justify-between items-center p-3 bg-white rounded border shadow-sm">
                                            <div>
                                                @php $brandModel = trim(($item->equipment->brand ?? '') . ' ' . ($item->equipment->model ?? '')); @endphp
                                                <p class="font-semibold text-gray-900">{{ $brandModel ?: ($item->equipment->name ?? 'Equipment') }}</p>
                                                <p class="text-xs text-gray-600">{{ $item->equipment->name ?? $item->equipment->category->name }}</p>
                                            </div>
                                            <div class="flex items-center gap-3">
                                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-50 text-blue-800">Qty: {{ $item->quantity_requested }}</span>
                                            </div>
                                        </div>
                                    @endforeach
                                </div>
                            ...
                            @if(in_array($reservation->status, ['pending', 'approved']))
                                <div class="mt-4 p-4 bg-red-50 border border-red-200 rounded-md">
                                    <div class="flex items-center justify-between">
                                        ...
                                        <button onclick="showCancelConfirmation('{{ $reservation->id }}', '{{ $reservation->reservation_code }}')"
                            ...
                            @if($reservation->status === 'pending')
                                <div class="mt-4 p-4 bg-blue-50 border border-blue-200 rounded-md">
                                    ...
                                    <div class="text-sm text-blue-800 space-y-2">
                                                ...
                                                <p><strong>Track Status:</strong> Check back here anytime using your reservation code <code class="bg-blue-100 px-1 rounded text-xs">{{ $reservation->reservation_code }}</code> to see updates.</p>
                                    ...
                                    <div class="flex items-start gap-3">
                                        ...
                                        <div class="flex-1">
                                            ...
                                            <div class="text-sm text-green-800 space-y-2">
                                                ...
                                                <p><strong>Return Schedule:</strong> Equipment must be returned by {{ $reservation->return_date->format('M d, Y') }} during business hours.</p>
    ...
    <script>
        ...
        function openEditReservationModal(){
            ...
            popup.querySelector('#edit_save').onclick = async ()=>{
                ...
                const existingIds = @json($reservation ? $reservation->items->pluck('id') : collect());
