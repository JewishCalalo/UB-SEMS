            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-4 sm:p-8">
                    ...
                    <div class="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 items-center">
                            ...
                            <div class="md:col-span-2 flex items-center justify-end gap-3">
                                <span class="text-2xl md:text-3xl font-bold text-blue-600 break-all">{{ $reservation->reservation_code }}</span>
                                <button type="button" id="copyReservationCodeBtn" data-code="{{ $reservation->reservation_code }}" 
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <p class="text-sm text-gray-900">
                                    {{ $reservation->borrow_date->format('F d, Y') }}
                                    @if($reservation->borrow_time)
                                        <span class="text-gray-600">at {{ \Carbon\Carbon::parse($reservation->borrow_time)->format('g:i A') }}</span>
                                    @endif
                                </p>
                            </div>
                            <div>
                                ...
                                <p class="text-sm text-gray-900">
                                    {{ $reservation->return_date->format('F d, Y') }}
                                    @if($reservation->return_time)
                                        <span class="text-gray-600">at {{ \Carbon\Carbon::parse($reservation->return_time)->format('g:i A') }}</span>
                                    @endif
                                </p>
                            </div>
                        </div>
                                    ...
                                    @foreach($reservation->items as $item)
                                        ...
                                        <tr>
                                            <td class="px-3 sm:px-6 py-4">
                                                <div class="text-sm font-medium text-gray-900">{{ $brandModel ?: ($item->equipment->name ?? 'Equipment') }}</div>
                                                @if($item->equipment->name && $brandModel)
                                                    <div class="text-xs text-gray-500">{{ $item->equipment->name }}</div>
                                                @endif
                                            </td>
                                            <td class="px-3 sm:px-6 py-4 text-sm text-gray-900">
                                                {{ $item->equipment->category->name }} / {{ optional($item->equipment->equipmentType)->name ?? 'N/A' }}
                                            </td>
                                            <td class="px-3 sm:px-6 py-4 text-sm text-gray-900">
                                                @if(!is_null($item->quantity_approved) && $item->quantity_approved > 0)
                                                    {{ $item->quantity_approved }} <span class="text-xs text-gray-500">(Approved)</span>
                                                @else
                            ...
                            @if($reservation->pickup_date && in_array($reservation->status, ['approved','picked_up','returned','completed']))
                                <div class="flex items-start">
                                    ...
                                    <div class="ml-3">
                                        ...
                                        <p class="text-sm text-gray-600">{{ \Carbon\Carbon::parse($reservation->pickup_date)->format('M d, Y') }}</p>
                                    </div>
                                </div>
                            @endif
                            ...
                            @if($reservation->status === 'cancelled')
                                <div class="flex items-start">
                                    ...
                                    <div class="ml-3">
                                        ...
                                        <p class="text-sm text-gray-600">{{ $reservation->cancelled_at ? $reservation->cancelled_at->format('M d, Y g:i A') : '—' }}</p>
                                    </div>
                                </div>
                            @endif
                            ...
                            @if($reservation->completed_at)
                                <div class="flex items-start">
                                    ...
                                    <div class="ml-3">
                                        ...
                                        <p class="text-sm text-gray-600">{{ $reservation->completed_at->format('M d, Y g:i A') }}</p>
                                    </div>
                                </div>
                            @endif
                        </div>
                    ...
                    <div class="flex flex-col sm:flex-row gap-4 justify-center">
                        ...
                        <button type="button"
                                onclick="showCancelConfirmation('{{ $reservation->id }}', '{{ $reservation->reservation_code }}')"
