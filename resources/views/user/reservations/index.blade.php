            ...
            <div class="bg-white rounded-lg shadow-sm overflow-hidden">
                                ...
                                @foreach($reservations as $reservation)
                                    <tr class="transition-shadow hover:shadow-md hover:bg-gray-50">
                                        @if(Auth::user()->isManager() || Auth::user()->isAdmin())
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                ...
                                                <div class="text-sm font-medium text-gray-700">
                                                    {{ $reservation->user ? $reservation->user->email : ($reservation->email ?? 'No email') }}
                                                </div>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="flex items-center">
                                                <div class="flex-shrink-0 h-10 w-10">
                                                    @if($reservation->items->first() && $reservation->items->first()->equipment->primary_image)
                                                        <img class="h-10 w-10 rounded-full object-cover" 
                                                             src="{{ $reservation->items->first()->equipment->primary_image->url }}" 
                                                             alt="{{ $reservation->items->first()->equipment->name }}">
                                                    @else
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            ...
                                            @endphp
                                            <span class="px-3 py-1.5 text-xs font-semibold rounded-full {{ $statusColor }} shadow-sm">
                                            ...
                                            @if(Auth::user()->isManager() || Auth::user()->isAdmin())
                                                @if($reservation->status === 'pending')
                                                    <button type="button" 
                                                            ...
                                                            onclick="showApproveConfirmation('{{ $reservation->id }}', '{{ $reservation->user ? $reservation->user->name : ($reservation->name ?? 'Guest User') }}')"
                                                    ...
                                                    <button type="button" 
                                                            id="deny_btn_{{ $reservation->id }}"
                                                            onclick="showDenyConfirmation('{{ $reservation->id }}', '{{ $reservation->user ? $reservation->user->name : ($reservation->name ?? 'Guest User') }}')"
                                                    ...
                                                    <form id="deny-form-{{ $reservation->id }}" 
                                                          method="POST" 
                                                          action="{{ route('reservations.update', $reservation) }}" 
                                                          class="hidden">
                                                        @csrf
                                                ...
                                                @elseif($reservation->status === 'approved')
                                                    <button type="button" 
                                                            id="picked_up_btn_{{ $reservation->id }}"
                                                            onclick="showPickedUpConfirmation('{{ $reservation->id }}', '{{ $reservation->user ? $reservation->user->name : ($reservation->name ?? 'Guest User') }}')"
                                                    ...
                                                    <form id="picked-up-form-{{ $reservation->id }}" 
                                                          method="POST" 
                                                          action="{{ route('reservations.update', $reservation) }}" 
                                                          class="hidden">
                                                        @csrf
                                                ...
                                                @elseif($reservation->status === 'picked_up')
                                                    <button type="button" 
                                                            id="returned_btn_{{ $reservation->id }}"
                                                            onclick="showReturnedConfirmation('{{ $reservation->id }}', '{{ $reservation->user ? $reservation->user->name : ($reservation->name ?? 'Guest User') }}')"
                                                    ...
                                                    <form id="returned-form-{{ $reservation->id }}" 
                                                          method="POST" 
                                                          action="{{ route('reservations.update', $reservation) }}" 
                                                          class="hidden">
                                                        @csrf
                ...
                @else
                    <div class="text-center py-12">
                        ...
                        @if(!Auth::user()->isManager() && !Auth::user()->isAdmin())
                            <div class="mt-6">
                                <a href="{{ route('reservations.create') }}" class="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-red-600 hover:bg-red-700">
                                    Make Reservation
                                </a>
                            </div>
                        @endif
