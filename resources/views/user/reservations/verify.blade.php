...
<head>
    ...
    <title>{{ config('app.name', 'SEMS') }} - Verify Reservation</title>
    ...
    <div class="min-h-screen flex flex-col justify-center items-center px-4 sm:px-6 lg:px-8 py-6 sm:py-0 relative">
        ...
        <div class="w-full max-w-sm px-4 sm:px-6 py-6 sm:py-8 bg-white shadow-md rounded-lg border border-gray-100 login__card relative z-10">
            <div class="mb-4 text-center">
                ...
                <p class="text-sm sm:text-base text-gray-600 mt-1">We sent a 6‑digit code to <strong>{{ $reservation->email }}</strong>. Enter it below to verify your email.</p>
            </div>
            ...
            <form id="verifyForm" method="POST" action="{{ route('reservations.verify.submit', $reservation) }}" class="space-y-5">
                @csrf
