<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="bg-white border border-gray-200 rounded-lg shadow-sm p-6">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            ...
                            <p class="text-2xl font-bold text-yellow-600">{{ $pendingItems }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white border border-gray-200 rounded-lg shadow-sm p-6">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            ...
                            <p class="text-2xl font-bold text-green-600">{{ $replacedItems }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white border border-gray-200 rounded-lg shadow-sm p-6">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            ...
                            <p class="text-2xl font-bold text-red-600">{{ $notReplacedItems }}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                ...
                <div class="bg-white border border-orange-200 rounded-lg shadow-sm p-6">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-sm font-medium text-gray-600">Lost</p>
                            <p class="text-2xl font-bold text-orange-600">{{ $lostCount }}</p>
                        </div>
                ...
                <div class="bg-white border border-yellow-200 rounded-lg shadow-sm p-6">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-2xl font-bold text-yellow-600">{{ $notReturnedCount }}</p>
                        </div>
                        <div class="p-2 bg-yellow-100 rounded-lg">
                            <svg class="w-6 h-6 text-yellow-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
            ...
            <div id="searchFilterContent" class="bg-white border border-gray-200 rounded-lg shadow-sm mb-6" style="display: none;">
                <div class="p-6">
                    <form method="GET" action="{{ route('missing-equipment.index') }}" class="space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            ...
                            <div class="space-y-2">
                                ...
                                <select name="incident_type" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white text-sm font-medium transition-all">
                                    ...
                                    <option value="lost" {{ request('incident_type') === 'lost' ? 'selected' : '' }}>Lost</option>
                                    <option value="damaged" {{ request('incident_type') === 'damaged' ? 'selected' : '' }}>Damaged</option>
                                    <option value="not_returned" {{ request('incident_type') === 'not_returned' ? 'selected' : '' }}>Not Returned</option>
                                </select>
                            </div>
                            ...
                            <div class="space-y-2">
                                ...
                                <select name="replacement_status" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white text-sm font-medium transition-all">
                                    ...
                                    <option value="replaced" {{ request('replacement_status') === 'replaced' ? 'selected' : '' }}>Replaced</option>
                                    <option value="not_replaced" {{ request('replacement_status') === 'not_replaced' ? 'selected' : '' }}>Not Replaced</option>
                                </select>
                            </div>
                        ...
                        @if($stolenLostItems->count() > 0)
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    @foreach($stolenLostItems as $item)
                                        <tr class="hover:bg-gray-50">
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold text-gray-900">
                                                <div class="flex items-center">
                                                    <div>
                                                        <div class="text-sm font-bold text-gray-900">
                                                            {{ $item->equipmentInstance->equipment->brand }} / {{ $item->equipmentInstance->equipment->model }}
                                                        </div>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div>
                                                    <div class="text-sm font-bold text-gray-900">{{ $item->borrower_name }}</div>
                                                    <div class="text-sm font-medium text-gray-700">{{ $item->borrower_email }}</div>
                                                </div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-{{ $item->incident_type_color }}-100 text-{{ $item->incident_type_color }}-800">
                                                    {{ $item->incident_type_text }}
                                                </span>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-{{ $item->replacement_status_color }}-100 text-{{ $item->replacement_status_color }}-800">
                                                    {{ $item->replacement_status_text }}
                                                </span>
                                            </td>
                                            <td class="px-8 py-4 whitespace-nowrap text-sm font-medium">
                                                <div class="flex items-center space-x-2">
                                                    <a href="{{ route('missing-equipment.show', $item) }}" 
                                                    ...
                                                    <a href="{{ route('missing-equipment.edit', $item) }}" 
                                                    ...
                                                    @if($item->replacement_status === 'pending')
                                                        <button type="button" onclick="confirmMarkReplaced({{ $item->id }})" 
                                                        ...
                                                        <button type="button" onclick="confirmMarkNotReplaced({{ $item->id }})" 
    ...
    <script>
        ...
        function releaseEmail(email){
            fetch('/api/blacklist/release', {method:'POST', headers:{'Content-Type':'application/json','X-CSRF-TOKEN':'{{ csrf_token() }}'}, body: JSON.stringify({email})})
        ...
        function openGenerateReportModal() {
            Swal.fire({
                ...
                html: `
                    ...
                    <form id="report-form" method="GET" action="{{ route('missing-equipment.generate-pdf') }}" class="space-y-4">
            ...
            window.submitAdminMissingReport = function(){
                ...
                const base = fmt === 'excel' ? '{{ route('missing-equipment.export-excel') }}' : '{{ route('missing-equipment.generate-pdf') }}';
        ...
        function confirmMarkReplaced(itemId) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    form.method = 'POST';
                    form.action = `{{ route('missing-equipment.mark-replaced', ':id') }}`.replace(':id', itemId);
        ...
        function confirmMarkNotReplaced(itemId) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    const form = document.createElement('form');
                    form.method = 'POST';
                    form.action = `{{ route('missing-equipment.mark-not-replaced', ':id') }}`.replace(':id', itemId);
