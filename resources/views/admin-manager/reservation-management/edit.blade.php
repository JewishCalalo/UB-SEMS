    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            @else
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6">
                                            <form method="POST" action="{{ route('reservation-management.update', $reservation) }}" class="space-y-6">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <select name="status" id="status" required
                                        ...
                                        @foreach(['pending', 'approved', 'denied', 'picked_up', 'returned', 'completed', 'cancelled'] as $status)
                                ...
                                @if(!$reservation->createdBy || $reservation->user)
                                    ...
                                    <div>
                                        ...
                                        <input type="date" name="pickup_date" id="pickup_date" 
                                               ...
                                               min="{{ $reservation->borrow_date->format('Y-m-d') }}"
                                               max="{{ $reservation->return_date->format('Y-m-d') }}"
                                               class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                                        <p class="mt-1 text-sm text-gray-500">Set pickup date when approving reservation (between {{ $reservation->borrow_date->format('M d, Y') }} and {{ $reservation->return_date->format('M d, Y') }})</p>
                                    </div>
                                @else
