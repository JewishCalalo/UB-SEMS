<x-app-layout>
                            ...
                            <div class="flex items-start space-x-4">
                                @if($equipment->images->count() > 0)
                                    <img class="h-24 w-24 rounded-lg object-cover" 
                                         src="{{ $equipment->images->first()->url }}" 
                                ...
                                @endif
                                <div class="flex-1">
                                    <h2 class="text-2xl font-bold text-gray-900">{{ $equipment->display_name }}</h2>
                                    <p class="text-gray-600">{{ $equipment->brand ?: 'No brand specified' }}</p>
                                ...
                                <div>
                                    ...
                                    <div class="space-y-2">
                                        <div class="flex justify-between">
                                            ...
                                            <span class="text-sm font-medium text-gray-900">{{ $equipment->quantity_total }}</span>
                                        </div>
                                        <div class="flex justify-between">
                                            <span class="text-sm text-gray-600">Available:</span>
                                            <span class="text-sm font-medium text-gray-900">{{ $equipment->quantity_available }}</span>
                                        </div>
                                        <div class="flex justify-between">
                                            <span class="text-sm text-gray-600">In Use:</span>
                                            <span class="text-sm font-medium text-gray-900">{{ $equipment->quantity_total - $equipment->quantity_available }}</span>
                                        </div>
                                    ...
                                    <div class="space-y-2">
                                        @if($equipment->equipmentType)
                                            <div class="flex justify-between">
                                                ...
                                                <span class="text-sm font-medium text-gray-900">{{ $equipment->equipmentType->name }}</span>
                                            </div>
                                        @endif
                                        @if($equipment->model)
                                            <div class="flex justify-between">
                                        ...
                                        <div class="flex justify-between">
                                            ...
                                            <span class="text-sm font-medium text-gray-900">{{ $equipment->location ?: 'Not specified' }}</span>
                                        </div>
                                        <div class="flex justify-between">
                                            <span class="text-sm text-gray-600">Status:</span>
                                            <span class="text-sm font-medium text-gray-900">{{ $equipment->is_active ? 'Active' : 'Inactive' }}</span>
                                        </div>
                                        @if($equipment->purchase_date)
                                            <div class="flex justify-between">
                                                <span class="text-sm text-gray-600">Purchase Date:</span>
                                                <span class="text-sm font-medium text-gray-900">{{ $equipment->purchase_date->format('M d, Y') }}</span>
                                            </div>
                                        @endif
                                    </div>
                                </div>
                    ...
                    @if($maintenanceRecords->count() > 0)
                        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
                            <div class="p-6">
                                ...
                                <div class="space-y-3">
                                    @foreach($maintenanceRecords->take(3) as $record)
                                        <div class="border-l-4 border-green-500 pl-3">
                                            <p class="text-sm font-medium text-gray-900">{{ $record->maintenance_type }}</p>
                                            <p class="text-xs text-gray-600">{{ $record->maintenance_date->format('M d, Y') }}</p>
                                            <p class="text-xs text-gray-500">{{ Str::limit($record->description, 50) }}</p>
                                        </div>
                                    @endforeach
                                </div>
                                <div class="mt-3">
                    ...
                    <div class="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden">
                        <div class="p-6">
                            ...
                            <div class="space-y-3">
                                ...
                                <button type="button" 
                                        onclick="showDeleteEquipmentConfirmation('{{ $equipment->id }}', '{{ $equipment->display_name }}')"
                ...
                <div class="p-6">
                    <div class="flex justify-between items-center mb-6">
                        ...
                        <div class="flex items-center gap-3">
                            <div class="text-sm font-semibold text-gray-700">Total: {{ $equipment->instances->count() }} instances</div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse($equipment->instances as $instance)
                                    <tr class="hover:bg-gray-50">
                                        <td class="px-6 py-4 whitespace-nowrap text-sm"><input type="checkbox" class="instance-checkbox w-4 h-4 text-red-600 focus:ring-red-500 border-gray-300 rounded" value="{{ $instance->id }}"></td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $instance->instance_code }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                                            <span class="inline-flex px-2 py-0.5 text-xs rounded-full {{ $instance->condition === 'excellent' ? 'bg-green-100 text-green-800' : ($instance->condition === 'good' ? 'bg-blue-100 text-blue-800' : ($instance->condition === 'fair' ? 'bg-yellow-100 text-yellow-800' : ($instance->condition === 'needs_repair' ? 'bg-orange-100 text-orange-800' : ($instance->condition === 'damaged' ? 'bg-red-100 text-red-800' : 'bg-gray-100 text-gray-800')))) }}">{{ ucfirst(str_replace('_',' ', $instance->condition)) }}</span>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm">@php $status = $equipment->getInstanceReservationStatus($instance->id); $isAvailable = $status === 'Available'; @endphp {{ $isAvailable ? 'Yes' : 'No' }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm">@php $status = $equipment->getInstanceReservationStatus($instance->id); @endphp @if($status === 'Reserved')<span class="inline-flex px-2 py-0.5 text-xs rounded-full bg-purple-100 text-purple-800">Reserved</span>@elseif($status === 'Borrowed')<span class="inline-flex px-2 py-0.5 text-xs rounded-full bg-red-100 text-red-800">Borrowed</span>@elseif($status === 'Unavailable')<span class="inline-flex px-2 py-0.5 text-xs rounded-full bg-gray-100 text-gray-800">Unavailable</span>@else<span class="inline-flex px-2 py-0.5 text-xs rounded-full bg-green-100 text-green-800">Available</span>@endif</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm">{{ $instance->has_reservation_history ? 'Yes' : 'No' }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm">{{ $instance->location ?? '—' }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm">@php $reservations = $instance->getCurrentReservationDates(); @endphp @if($reservations->count() > 0) @foreach($reservations as $reservation) <div class="text-xs"><span class="text-black">{{ \Carbon\Carbon::parse($reservation['borrow_date'])->format('M d') }} - {{ \Carbon\Carbon::parse($reservation['return_date'])->format('M d') }}</span> <span class="ml-1 px-1 py-0.5 rounded text-xs {{ $reservation['is_current'] ? 'bg-red-100 text-red-800' : ($reservation['is_future'] ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-600') }}">{{ $reservation['is_current'] ? 'Current' : ($reservation['is_future'] ? 'Future' : 'Past') }}</span></div> @endforeach @else <span class="text-gray-400">—</span> @endif</td>
                                    </tr>
                                @empty
                                    <tr>
    ...
    <form id="delete-equipment-form" 
          method="POST" 
          action="{{ route('equipment-management.destroy', $equipment) }}" 
          class="hidden">
        @csrf
        @method('DELETE')
    </form>
    ...
    <script>
                    ...
                    Swal.fire({
                        ...
                        html: `
                            ...
                            <div class="flex justify-center mt-6">
                                <button type="button" class="px-6 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 transition-colors transform hover:scale-105" onclick="Swal.close(); window.location.href='{{ route('equipment-management.index') }}';">
                                    OK
                                </button>
                            </div>
                        `,
        ...
        function showAddInstanceModal() {
            Swal.fire({
                html: `
                    ...
                    <form id="addInstanceForm" method="POST" action="{{ route('instances.add', $equipment) }}" class="space-y-4">
                ...
                preConfirm: () => {
                    ...
                    if (form) {
                        ...
                        const buttonId = `add_instance_btn_{{ $equipment->id }}`;
                        
                        ActionHandler.handleAjaxAction(buttonId, "{{ route('instances.add', $equipment) }}", {
                            method: 'POST',
                            data: data,
        ...
        async function bulkDeleteInstances(){
            ...
            for(const id of selected){
                try{
                    const resp = await fetch(`{{ url('/instances') }}/${id}`, {method:'POST', headers:{'X-CSRF-TOKEN':'{{ csrf_token() }}','Accept':'application/json'}, body:new URLSearchParams({'_method':'DELETE'})});
        ...
        async function bulkRetireInstances(){
            ...
            for(const id of selected){
                try{
                    const resp = await fetch(`{{ url('/instances') }}/${id}/retire`, {method:'POST', headers:{'X-CSRF-TOKEN':'{{ csrf_token() }}','Accept':'application/json'}});
                    if(resp.ok){ success++; } else { failed++; }
                }catch(e){ failed++; }
            }
            const html = failed===0 ? `
