...
<head>
    ...
    <title>{{ config('app.name', 'SEMS') }} - Login</title>
    ...
    <div class="min-h-screen flex flex-col justify-center items-center px-4 sm:px-6 lg:px-8 py-6 sm:py-0 relative">
        ...
        <div class="w-full max-w-sm px-4 sm:px-6 py-6 sm:py-8 bg-white shadow-md rounded-lg border border-gray-100 login__card relative z-10">
            <div class="mb-6 text-center">
                <img src="{{ asset('images/ub-logo.png') }}" alt="SEMS Logo" class="mx-auto h-20 w-auto object-contain" />
            </div>
            <div class="mb-6 text-center">
            ...
            <!-- Session Status -->
            @if (session('status'))
                @if (session('status') === 'email-verification-sent-logout')
            ...
            <form method="POST" action="{{ route('login') }}" class="space-y-6" novalidate>
                ...
                <!-- User Type Selection -->
                {{-- <div>
                    <label for="user_type" class="block text-sm font-medium text-gray-700 mb-3">
                        Login as
                    </label>
                    <div class="grid grid-cols-2 gap-4">
                        <label class="relative flex cursor-pointer rounded-lg border-2 border-gray-300 p-4 shadow-sm focus:outline-none transition-all duration-200 hover:border-rose-300">
                            <input type="radio" name="user_type" value="manager" class="sr-only" checked>
                            <span class="flex flex-1">
                                <span class="flex flex-col">
                                    <span class="block text-sm font-medium text-gray-900">Manager</span>
                                    <span class="mt-1 flex items-center text-xs text-gray-500">Manage requests & equipment</span>
                                </span>
                            </span>
                        ...
                        <label class="relative flex cursor-pointer rounded-lg border-2 border-gray-300 p-4 shadow-sm focus:outline-none transition-all duration-200 hover:border-rose-300">
                            <input type="radio" name="user_type" value="admin" class="sr-only">
                            <span class="flex flex-1">
                                <span class="flex flex-col">
                                    <span class="block text-sm font-medium text-gray-900">Admin</span>
                                    <span class="mt-1 flex items-center text-xs text-gray-500">Full system access</span>
                                </span>
                            </span>
                            <span class="pointer-events-none absolute -inset-px rounded-lg border-2 border-transparent" aria-hidden="true"></span>
                        </label>
