<?php if($equipment->count() == 0): ?>
    <div class="text-center py-12">
        ...
        <p class="mt-1 text-sm text-gray-500">
            <?php if(request('search') || request('category') || request('equipment_type') || request('availability')): ?>
...
<?php else: ?>
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow duration-200 transform hover:-translate-y-0.5">
                ...
                <div class="relative aspect-w-16 aspect-h-9 bg-gray-100">
                    <?php if($item->availableCount <= 0): ?>
                        <button
                            onclick="notifyWhenAvailable(<?php echo e($item->id); ?>, '<?php echo e(addslashes($item->display_name)); ?>', event)"
                    ...
                    <?php if($item->images->count() > 0): ?>
                        <img src="<?php echo e($item->images->first()->url); ?>" 
                             alt="<?php echo e($item->display_name); ?>" 
                ...
                <div class="p-4">
                    <div class="flex justify-between items-start mb-2">
                        <h3 class="text-lg font-semibold text-gray-900 line-clamp-2"><?php echo e($item->display_name); ?></h3>
                        <span class="text-sm text-gray-500"><?php echo e($item->category->name); ?></span>
                    </div>
                    
                    <p class="text-sm text-gray-600 mb-3 line-clamp-2"><?php echo e(Str::limit($item->description, 100)); ?></p>
                    ...
                    <div class="flex items-center justify-between mb-2">
                        <div class="flex items-center space-x-2">
                            <?php if($item->availableCount > 0): ?>
                                <span class="inline-flex items-center px-3 py-1.5 rounded-full text-base font-semibold bg-green-100 text-green-800">
                                    Available (<?php echo e($item->availableCount); ?>)
                                </span>
                            <?php else: ?>
                        ...
                        <div class="flex items-center space-x-2">
                            <?php if($item->availableCount <= 0): ?>
                                ...
                                <button onclick="addToWishlist(<?php echo e($item->id); ?>, '<?php echo e(addslashes($item->display_name)); ?>', event)"
                            ...
                            <?php if($item->availableCount > 0): ?>
                                <button onclick="addToReservation(<?php echo e($item->id); ?>, '<?php echo e(addslashes($item->display_name)); ?>', <?php echo e($item->availableCount); ?>)" 
        ...
        <div class="mt-8">
            <nav class="flex items-center justify-between">
                <div class="flex-1 flex justify-between sm:hidden">
                    ...
                    <?php else: ?>
                        <button onclick="loadPage(<?php echo e($equipment->currentPage() - 1); ?>)" class="relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50">
                            Previous
                        </button>
                    <?php endif; ?>

                    <?php if($equipment->hasMorePages()): ?>
                        <button onclick="loadPage(<?php echo e($equipment->currentPage() + 1); ?>)" class="ml-3 relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50">
                            Next
                        </button>
                    <?php else: ?>
                ...
                <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                    <div>
                        <p class="text-sm text-gray-700">
                            Showing
                            <span class="font-medium"><?php echo e($equipment->firstItem()); ?></span>
                            to
                            <span class="font-medium"><?php echo e($equipment->lastItem()); ?></span>
                            of
                            <span class="font-medium"><?php echo e($equipment->total()); ?></span>
                            results
                        </p>
                    </div>
                    <div>
                        <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                            ...
                            <?php else: ?>
                                <button onclick="loadPage(<?php echo e($equipment->currentPage() - 1); ?>)" class="relative inline-flex items-center px-2 py-2 rounded-l-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50">
                                    <span class="sr-only">Previous</span>
                                    <svg class="h-5 w-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                                        <path fill-rule="evenodd" d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clip-rule="evenodd" />
                                    </svg>
                            ...
                            <?php $__currentLoopData = $equipment->getUrlRange(1, $equipment->lastPage()); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $page => $url): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <?php if($page == $equipment->currentPage()): ?>
                                    <span class="relative inline-flex items-center px-4 py-2 border border-gray-300 bg-blue-50 text-sm font-medium text-blue-600">
                                        <?php echo e($page); ?>

                                    </span>
                                <?php else: ?>
                                    <button onclick="loadPage(<?php echo e($page); ?>)" class="relative inline-flex items-center px-4 py-2 border border-gray-300 bg-white text-sm font-medium text-gray-700 hover:bg-gray-50">
                                        <?php echo e($page); ?>
                            ...
                            <?php if($equipment->hasMorePages()): ?>
                                <button onclick="loadPage(<?php echo e($equipment->currentPage() + 1); ?>)" class="relative inline-flex items-center px-2 py-2 rounded-r-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50">
    ...
    <?php endif; ?>
<?php endif; ?>
<?php /**PATH C:\Users\Bryan\SEMS\SEMSv25\resources\views/components/equipment-grid.blade.php ENDPATH**/ ?>